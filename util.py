...
def maskNLLLoss(output, target, mask, device):
    ...
    print("target unsqueeze: ", target.unsqueeze(2).size())
    ...
    print("crossEntropy squeeze: ", crossEntropy.squeeze(2).size())
...
def train_one_batch(input_seq,
    ...
    # print("ENCODER H", encoder_hidden.size())
    ...
    # print("DECODER H", decoder_hidden.size())
    ...
    if use_teacher_forcing:
        for i in range(max_target_len):
            ...
            output, current_hidden = decoder(decoder_input, decoder_hidden,
                                             encoder_output)
            # print("DECODER O:", output.size(), " H:", current_hidden.size())
            ...
            # print("DECODER H:", current_hidden.size())
    else:
        for i in range(max_target_len):
            # 执行一步decoder前馈，只计算了一个单词\
            # print("DECODER")
...
class GreedySearchDecoder(nn.Module):
    ...
    def forward(self, input_seq, input_length, output_length, sos_token=1):
        ...
        print("decoder_hidden: ", decoder_hidden.size(), 'output_length: ', output_length)
        ...
        for _ in range(output_length):
            ...
            print("current_hidden: ", current_hidden.size())
