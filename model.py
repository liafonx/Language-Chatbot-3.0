...
from Model.old_ChaoticLSTM import ChaoticLSTM
from Model.old_LeeOscillator import LeeOscillator
Lee = LeeOscillator()
# 前馈的encoder
class EncoderRNN(nn.Module):
    ...
    def __init__(self, hidden_size, embedding, n_layers=1, dropout=0):
        ...
        self.gru = nn.GRU(input_size=hidden_size,
                          ...
                          bidirectional=False,
                          batch_first=True)
        # self.gru1 = ChaoticGRU(hidden_size, hidden_size, Lee, True, True)
        # self.gru2 = ChaoticGRU(hidden_size, hidden_size, Lee, True, True)
        # self.lstm1 = ChaoticLSTM(hidden_size, hidden_size, Lee, False, True)
        # self.lstm2 = ChaoticLSTM(hidden_size, hidden_size, Lee, False, True)
    ...
    def forward(self, input_seq, input_length, hidden=None):
        ...
        # print("In EncoderRNN: embedded: ", embedded.size())
        # outputs1, hidden1 = self.gru1(embedded, initStates=hidden)
        # outputs, hidden2 = self.gru2(embedded, initStates=hidden1)
        # outputs1, (h1, c1) = self.lstm1(embedded, initStates=hidden)
        # print("In EncoderRNN: outputs1: ", outputs1)
        # outputs, (h2, c2) = self.lstm2(embedded, initStates=(h1, c1)) #[2 * ht, hinvt], [2 * ct, cinvt]
...
class LuongAttentionDecoderRNN(nn.Module):
    ...
    def __init__(self,
        ...
        self.gru = nn.GRU(input_size=hidden_size,
                          ...
                          dropout=(0 if n_layers == 1 else dropout),
                          batch_first=True)
        # self.gru1 = ChaoticGRU(hidden_size, hidden_size, Lee, True, False)
        # self.gru2 = ChaoticGRU(hidden_size, hidden_size, Lee, True, False)
        # self.lstm1 = ChaoticLSTM(hidden_size, hidden_size, Lee, True, False)
        # self.lstm2 = ChaoticLSTM(hidden_size, hidden_size, Lee, True, False)
    ...
    def forward(self, input_single_seq, last_hidden, encoder_output):
        ...
        # print("In LuongAttentionDecoderRNN: embedded: ", embedded.size())
        # print("In LuongAttentionDecoderRNN: encoder_output: ", encoder_output)
        # h = last_hidden[-1:]
        # h = torch.cat(h.split(1), dim=-1).squeeze(0)
        # c = torch.zeros(h.size()).to(h.device)
        # print("last_hidden: ", last_hidden.size())
        # current_output1, (h1, c1) = self.lstm1(embedded, initStates=(h, c))
        # current_output1, h1 = self.gru1(embedded, initStates=h)
        # print("In LuongAttentionDecoderRNN: current_output1: ", current_output1.size())
        # current_output2, (h2, c2) = self.lstm2(embedded, initStates= (h1, c1))
        # current_output1, h2 = self.gru1(embedded, initStates=h)
        # print("In LuongAttentionDecoderRNN: current_output2: ", current_output2.size())
        # current_hidden = torch.cat([h1.unsqueeze(0), h2.unsqueeze(0)], dim=0)
        # print("In LuongAttentionDecoderRNN: current_hidden: ", current_hidden.size())
        # current_output : [batch_size, 1, hidden_size]
        # current_hidden : [1, batch_size, hidden_size]
        current_output2, current_hidden = self.gru(embedded, last_hidden)
        # print("In LuongAttentionDecoderRNN: current_output2: ", current_output2)
        # 这里拿current_output当做current_hidden来参与attention运算
        # 当然，直接拿current_hidden也行
        attention_vector = self.attention(current_output2, encoder_output)
        # attention_vector : [batch_size, 1, hidden_size]
