    ...
    def forward(self, x, initStates=None):
        ...
        if initStates is None:
            if self.bidirection == True:
                device = x.device
                ht = torch.zeros(bs, self.hiddenSize, device=device)
                ct = torch.zeros(bs, self.hiddenSize, device=device)
                hinvt = torch.zeros(bs, self.hiddenSize, device=device)
                cinvt = torch.zeros(bs, self.hiddenSize, device=device)
            else:
                device = x.device
                ht = torch.zeros(bs, self.hiddenSize, device=device)
                ct = torch.zeros(bs, self.hiddenSize, device=device)
        ...
        for t in range(seqs):
            ...
            if self.chaotic == True:
                it = self.Lee.Sigmoid(gates[:, :self.hiddenSize])
                ft = self.Lee.Sigmoid(gates[:, self.hiddenSize:self.hiddenSize * 2])
                gt = self.Lee.Tanh(gates[:, self.hiddenSize * 2:self.hiddenSize * 3])
                ot = self.Lee.Sigmoid(gates[:, self.hiddenSize * 3:])
        ...
        if self.bidirection == True:
            for t in range(seqs - 1, -1, -1):
                ...
                if self.chaotic == True:
                    ...
                    iinvt = self.Lee.Sigmoid(gatesInv[:, :self.hiddenSize])
                    finvt = self.Lee.Sigmoid(gatesInv[:, self.hiddenSize:self.hiddenSize * 2])
                    ginvt = self.Lee.Tanh(gatesInv[:, self.hiddenSize * 2:self.hiddenSize * 3])
                    oinvt = self.Lee.Sigmoid(gatesInv[:, self.hiddenSize * 3:])
...
if __name__ == "__main__":
    ...
    CLSTM = ChaoticLSTM(inputSize=46, hiddenSize=10, chaotic=False, bidirection=True)
    # Test the Bi-LSTM.
    # x = torch.randn((32, 10, 46))
    x = torch.randn((10, 32, 46))
    print(x.shape)
    print("Wi:", CLSTM.Wi.shape)
    output, (h, c) = CLSTM(x)
    print(output.shape)
    print(h.shape)
    print(c.shape)
    ...
    CLSTM = ChaoticLSTM(inputSize=46, hiddenSize=10, chaotic=True, bidirection=True)
    ...
    CLSTM = ChaoticLSTM(inputSize=46, hiddenSize=10, chaotic=True, bidirection=False)
    ...
    CLSTM = ChaoticLSTM(inputSize=46, hiddenSize=10, chaotic=False, bidirection=False)
    # Test the LSTM.
    # x = torch.randn((32, 10, 46))
    x = torch.randn((10, 32, 46))
    print(x.shape)
