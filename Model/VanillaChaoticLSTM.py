...
class VanillaChaoticLSTM(nn.Module):
    '''
    ...
    def __init__(self, inputSize, hiddenSize, chaotic = False, bidirection = False):
        ...
        super(VanillaChaoticLSTM, self).__init__()
    ...
    def forward(self, x, initStates = None):
        ...
        if initStates is None:
            if self.bidirection == True:
                ht, ct, hinvt, cinvt = (torch.zeros(bs, self.hiddenSize).to(cuda), torch.zeros(bs, self.hiddenSize).to(cuda), torch.zeros(bs, self.hiddenSize).to(cuda), torch.zeros(bs, self.hiddenSize).to(cuda))
            else:
                ht, ct = (torch.zeros(bs, self.hiddenSize).to(cuda), torch.zeros(bs, self.hiddenSize).to(cuda))
        else:
            if self.bidirection == True:
        ...
        for t in range(seqs):
            ...
            if self.chaotic == True:
                it, ft, gt, ot = (
                    self.Lee.Sigmoid(gates[:, :self.hiddenSize]).to(cuda),
                    self.Lee.Sigmoid(gates[:, self.hiddenSize:self.hiddenSize * 2]).to(cuda),
                    self.Lee.Tanh(gates[:, self.hiddenSize * 2:self.hiddenSize * 3]).to(cuda),
                    self.Lee.Sigmoid(gates[:, self.hiddenSize * 3:]).to(cuda)
                )
                # Compute the cell and hidden.
                ct = ft * ct + it * gt
                ht = ot * self.Lee.Tanh(ct).to(cuda)
            else:
                it, ft, gt, ot = (
                    torch.sigmoid(gates[:, :self.hiddenSize]).to(cuda),
                    torch.sigmoid(gates[:, self.hiddenSize:self.hiddenSize * 2]).to(cuda),
                    torch.tanh(gates[:, self.hiddenSize * 2:self.hiddenSize * 3]).to(cuda),
                    torch.sigmoid(gates[:, self.hiddenSize * 3:]).to(cuda)
                )
                # Compute the cell and hidden.
                ct = ft * ct + it * gt
                ht = ot * torch.tanh(ct).to(cuda)
            ...
            for t in range(seqs - 1, -1, -1):
                ...
                if self.chaotic == True:
                    ...
                    iinvt, finvt, ginvt, oinvt = (
                        self.Lee.Sigmoid(gatesInv[:, :self.hiddenSize]).to(cuda),
                        self.Lee.Sigmoid(gatesInv[:, self.hiddenSize:self.hiddenSize * 2]).to(cuda),
                        self.Lee.Tanh(gatesInv[:, self.hiddenSize * 2:self.hiddenSize * 3]).to(cuda),
                        self.Lee.Sigmoid(gatesInv[:, self.hiddenSize * 3:]).to(cuda)
                    )
                    # Compute the inverse cell and hidden.
                    cinvt = finvt * cinvt + iinvt * ginvt
                    hinvt = oinvt * self.Lee.Tanh(cinvt).to(cuda)
                else:
                    # Get the value of each inverse gate.
                    iinvt, finvt, ginvt, oinvt = (
                        torch.sigmoid(gatesInv[:, :self.hiddenSize]).to(cuda),
                        torch.sigmoid(gatesInv[:, self.hiddenSize:self.hiddenSize * 2]).to(cuda),
                        torch.tanh(gatesInv[:, self.hiddenSize * 2:self.hiddenSize * 3]).to(cuda),
                        torch.sigmoid(gatesInv[:, self.hiddenSize * 3:]).to(cuda)
                    )
                    # Compute the inverse cell and hidden.
                    cinvt = finvt * cinvt + iinvt * ginvt
                    hinvt = oinvt * torch.tanh(cinvt).to(cuda)
